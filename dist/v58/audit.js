...
exports.auditMapping = auditMapping;
const model_1 = require("./model");
function elementAction(action) {
    if (action.kind === 'press' && action.metadata?.globalKeyboard)
        return false;
    return ['click', 'fill', 'select', 'press', 'check', 'uncheck', 'hover', 'verify'].includes(action.kind);
}
function repeatedConditions(plan) {
    const groups = new Map();
    const stack = [];
    plan.actions.forEach((action, index) => {
        if (action.kind === 'ifStart')
            stack.push({ expression: action.condition ?? action.value ?? action.name, start: index, actions: [] });
        else if (action.kind === 'ifEnd') {
            const branch = stack.pop();
            if (branch) {
                const entries = groups.get(branch.expression) ?? [];
                entries.push({ index: branch.start, actions: branch.actions });
                groups.set(branch.expression, entries);
            }
        }
        else
            for (const branch of stack)
                branch.actions.push(`${action.kind}:${action.locatorId ?? action.name}`);
    });
...
function auditMapping(mapping) {
    ...
    for (const { plan, action } of allActions) {
        if (elementAction(action) && !action.locator)
            issues.push({ severity: 'error', code: 'ELEMENT_ACTION_WITHOUT_LOCATOR', message: `${action.kind} has no locator evidence`, planId: plan.id, actionId: action.id });
        if (action.kind === 'setData' && !action.key)
            issues.push({ severity: 'error', code: 'DATA_SET_WITHOUT_KEY', message: 'data.set has no target key', planId: plan.id, actionId: action.id });
        if (action.metadata?.unresolvedElement)
            issues.push({ severity: 'warning', code: 'UNRESOLVED_TECHNICAL_ACTION', message: 'raw technical action retained as non-executable metadata because no UI control evidence was linked', planId: plan.id, actionId: action.id });
        if (action.kind === 'select' && action.value === undefined)
            issues.push({ severity: 'warning', code: 'DROPDOWN_WITHOUT_VALUE', message: 'dropdown selection has no value', planId: plan.id, actionId: action.id });
        if (action.locator && action.locator.candidates.length === 0)
            issues.push({ severity: 'error', code: 'LOCATOR_WITHOUT_CANDIDATES', message: 'locator descriptor has no candidates', planId: plan.id, actionId: action.id });
    }
    for (const plan of mapping.plans) {
        const ids = plan.actions.map((action) => action.id);
        if (new Set(ids).size !== ids.length)
            issues.push({ severity: 'error', code: 'DUPLICATE_ACTION_ID', message: 'plan contains duplicate action ids', planId: plan.id });
        ...
        if ((0, model_1.canonicalJson)(sourceSetPositions) !== (0, model_1.canonicalJson)(sorted))
            issues.push({ severity: 'error', code: 'DATA_SET_REORDERED', message: 'data.set source ordinals are not preserved', planId: plan.id });
        let depth = 0;
        for (const action of plan.actions) {
            ...
            if (action.kind === 'ifEnd')
                depth -= 1;
            if (depth < 0)
                issues.push({ severity: 'warning', code: 'UNBALANCED_IF_END', message: 'ifEnd appeared without an open condition', planId: plan.id, actionId: action.id });
        }
        if (depth !== 0)
            issues.push({ severity: 'warning', code: 'UNBALANCED_IF', message: `condition depth ended at ${depth}`, planId: plan.id });
    }
    const strategyCounts = {};
    for (const locator of mapping.locators)
        for (const candidate of locator.candidates)
            strategyCounts[candidate.kind] = (strategyCounts[candidate.kind] ?? 0) + 1;
