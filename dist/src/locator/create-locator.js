function scopedContainer(container, scope) {
    if (!scope)
        return container;
    if (scope.css)
        return container.locator?.(scope.css) ?? container;
    if (scope.xpath)
        return container.locator?.(`xpath=${scope.xpath}`) ?? container;
    if (scope.text && container.getByText) {
        return container.getByText(scope.text, { exact: true });
    }
    return container;
}
export function createLocator(container, candidate) {
...
//# sourceMappingURL=create-locator.js.map