import { DEFAULT_FIELD_REF_ATTRIBUTES, } from './model.js';
...
function effectiveOccurrence(spec) {
...
function withCommon(candidate, occurrence, scope) {
    return {
        ...candidate,
...
function addRole(roles, seen, role) {
    if (!role?.role.trim())
        return;
    const normalized = {
...
function collectRoles(spec) {
    const roles = [];
    const seen = new Set();
    addRole(roles, seen, spec.role);
    for (const role of spec.roleAlternates ?? [])
        addRole(roles, seen, role);
...
export function buildLocatorCandidates(spec, options = {}) {
    const config = {
    ...
    for (const candidate of candidates.sort((left, right) => right.score - left.score)) {
        ...
        if (!deduped.has(identity))
            deduped.set(identity, candidate);
    }
