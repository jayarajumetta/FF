...
function actionReads(action) {
    ...
    if (action.condition) {
        const condition = typeof action.condition === 'string'
            ...
            : action.condition;
        for (const key of collectConditionVariables(condition))
            keys.add(key);
    }
    return keys;
}
function sanitizedCaptureName(action, index) {
 ...
 */
export function planDataSetFooter(actions) {
    const body = [];
    const deferred = [];
    const audit = [];
...
//# sourceMappingURL=data-footer.js.map