...
function actionLines(action, locatorName) {
    switch (action.kind) {
        ...
        case 'dataSet':
            if (!action.dataKey)
                throw new Error(`dataSet action ${action.id} has no dataKey.`);
...
function renderAction(action, locatorName, includeAuditComments) {
...
function captureLines(deferred) {
...
function formatObjectConstant(name, value, typeName) {
    const serialized = json(value).split('\n');
    if (serialized.length === 1)
        return [`const ${name} = ${serialized[0]} satisfies ${typeName};`];
    return [
 ...
 */
export function generatePlaywrightScenario(model, options = {}) {
    ...
    for (const action of normalized.actions) {
        ...
        if (!bodyIds.has(action.id))
            continue;
        lines.push(...indentBlock(renderAction(action, locatorByAction.get(action.id), includeAuditComments), 2));
        lines.push('');
    }
