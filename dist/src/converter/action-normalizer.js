...
function isGeneratedTab(action) {
...
function isRedundantBlankDropdownPriming(current, next) {
    if (!next || current.kind !== 'select' || next.kind !== 'select')
        return false;
    if (!sameTarget(current, next) || conditionIdentity(current) !== conditionIdentity(next))
        return false;
...
function isSafelyDuplicateClick(previous, current) {
    if (!previous || previous.kind !== 'click' || current.kind !== 'click')
        return false;
    if (!sameTarget(previous, current) || conditionIdentity(previous) !== conditionIdentity(current))
        return false;
    if (current.metadata?.keep === true)
        return false;
    const sameRawStep = Boolean(previous.metadata?.sourceStepId &&
 ...
 */
export function normalizeScenario(model) {
...
//# sourceMappingURL=action-normalizer.js.map