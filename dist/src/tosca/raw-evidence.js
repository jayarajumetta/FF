...
function integerByNames(properties, names) {
...
function collectProperties(attributes, body) {
...
function locatorFromProperties(properties, sourceFile) {
    ...
    if (!controlName && !fieldRef && !id && !customXPath)
        return undefined;
    return {
        ...(sourceFile === undefined ? {} : { sourceFile }),
        ...(valueByNames(properties, ['Surrogate', 'Guid', 'GUID', 'UniqueId'])
            ? { entityGuid: valueByNames(properties, ['Surrogate', 'Guid', 'GUID', 'UniqueId']) }
            : {}),
        ...(valueByNames(properties, ['ModuleGuid', 'ReferencedModule', 'ModuleId'])
            ? { moduleGuid: valueByNames(properties, ['ModuleGuid', 'ReferencedModule', 'ModuleId']) }
            : {}),
        ...(valueByNames(properties, ['ModuleName', 'ParentModule'])
            ? { moduleName: valueByNames(properties, ['ModuleName', 'ParentModule']) }
            : {}),
...
function actionFromProperties(properties, sourceFile) {
    ...
    if (!actionMode && value === undefined && !condition)
        return undefined;
    return {
        ...(sourceFile === undefined ? {} : { sourceFile }),
        ...(valueByNames(properties, ['Surrogate', 'Guid', 'GUID', 'UniqueId'])
        ...
        ...(valueByNames(properties, ['StepName', 'TestStepName'])
            ? { stepName: valueByNames(properties, ['StepName', 'TestStepName']) }
            : {}),
        ...(valueByNames(properties, ['Name', 'ControlName', 'TechnicalName'])
            ? { controlName: valueByNames(properties, ['Name', 'ControlName', 'TechnicalName']) }
            : {}),
        ...
        ...(integerByNames(properties, ['Order', 'Position', 'Sequence']) === undefined
            ? {}
            : { order: integerByNames(properties, ['Order', 'Position', 'Sequence']) }),
        properties,
    };
}
export function extractRawToscaEvidence(xml, sourceFile) {
    const locators = [];
    const actions = [];
...
function flattenObject(value, output, seen) {
...
export function extractRawToscaEvidenceFromObject(value, sourceFile) {
    const objects = [];
    flattenObject(value, objects, new Set());
    const locators = [];
    const actions = [];
...
export function locatorSpecFromRawEvidence(evidence, app, key = evidence.controlName ?? evidence.fieldRef ?? evidence.id ?? 'toscaControl') {
...
//# sourceMappingURL=raw-evidence.js.map