 ...
 */
export async function performDomFallback(page, spec, action, value, options = {}) {
    ...
    for (const frameRecord of enumerateFrames(page, spec.frame)) {
        ...
        for (const candidate of candidates) {
            ...
            try {
                const result = await frameRecord.frame.evaluate((payload) => {
                    ...
                    const allRoots = (root) => {
                        ...
                        while (stack.length > 0) {
                            ...
                            if (!current || !('querySelectorAll' in current))
                                continue;
                    ...
                    const queryXPath = (root, xpath) => {
                        const documentNode = root instanceof Document ? root : root.ownerDocument;
                        if (!documentNode)
                            return [];
                        const output = [];
                        try {
                            const result = documentNode.evaluate(xpath, root, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
                    ...
                    const findWithin = (scope) => {
                        ...
                        if (descriptor.selector)
                            return queryCss(scope, descriptor.selector);
                        if (descriptor.kind === 'label' && descriptor.value) {
                            ...
                            for (const label of queryCss(scope, 'label')) {
                                if (normalize(label.textContent) !== expected)
                                    continue;
                            ...
                            for (const element of queryCss(scope, '[aria-label]')) {
                                if (normalize(element.getAttribute('aria-label')) === expected)
                                    output.push(element);
                            }
                            return output;
                        }
                        if (descriptor.kind === 'role' && descriptor.role) {
                            ...
                            const candidates = queryCss(scope, '[role],button,a[href],input,select,textarea,option,mat-select,mat-option');
                            return candidates.filter((element) => {
                                if (implicitRole(element) !== descriptor.role)
                                    return false;
                                if (!expectedName)
                                    return true;
                        ...
                        if (descriptor.kind === 'text' && descriptor.value) {
                            const expected = normalize(descriptor.value);
                            return queryCss(scope, 'button,a,input,option,mat-option,[role],label,span,div').filter((element) => {
                                const actual = accessibleName(element);
                                return descriptor.exact === false
                    ...
                    switch (payload.action) {
                        ...
                        case 'select': {
                            if (element instanceof HTMLSelectElement) {
                                const option = Array.from(element.options).find((item) => item.value === actionValue || normalize(item.textContent) === normalize(actionValue));
                                if (!option)
                                    return { ok: false, count: matches.length, selectedIndex: index, detail: `Native option not found: ${actionValue}` };
                        ...
                        case 'press': {
                            if (element instanceof HTMLElement)
                                element.focus();
                            const init = { key: actionValue, bubbles: true, composed: true };
...
//# sourceMappingURL=dom-fallback.js.map