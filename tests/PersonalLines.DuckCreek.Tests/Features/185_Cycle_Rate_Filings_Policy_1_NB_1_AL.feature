@PL_DC @CYCLE @rate_filing_new_business @AL @automated @canonical @artifact_v38 @regression @rate_filings
Feature: Personal Lines Duck Creek - Cycle Rate Filings Policy 1 NB 1
  As a Personal Lines Duck Creek policy processing user
  I want to complete the attached Motorcycle rate filing new business workflow
  ...
  Scenario Outline: Create and verify a Motorcycle rate filing new business for <state>
    ...
    Examples:
      ...
      | 185_cycle_rate_filings_policy_1_nb_1_al | Alabama | AL | Motorcycle | Rate Filing New Business | Current |
