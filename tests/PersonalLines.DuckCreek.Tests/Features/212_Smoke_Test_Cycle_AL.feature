@PL_DC @CYCLE @smoke_test @AL @automated @canonical @artifact_v38 @smoke
Feature: Personal Lines Duck Creek - Smoke Test Cycle
  As a Personal Lines Duck Creek policy processing user
  I want to complete the attached Motorcycle smoke test workflow
  ...
  Scenario Outline: Create and verify a Motorcycle smoke test for <state>
    ...
    Examples:
      ...
      | 212_smoke_test_cycle_al | Alabama | AL | Motorcycle | Smoke Test | Current |
