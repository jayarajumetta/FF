@PL_DC @CYCLE @new_business_with_prior_effective_date @AL @automated @canonical @artifact_v38 @regression
Feature: Personal Lines Duck Creek - Cycle Rate Filings Policy 3 NB Prior Eff Date
  As a Personal Lines Duck Creek policy processing user
  I want to complete the attached Motorcycle new business with prior effective date workflow
  ...
  Scenario Outline: Create and verify a Motorcycle new business with prior effective date for <state>
    ...
    Examples:
      ...
      | 186_cycle_rate_filings_policy_3_nb_prior_eff_date_al | Alabama | AL | Motorcycle | New Business with Prior Effective Date | Prior |
