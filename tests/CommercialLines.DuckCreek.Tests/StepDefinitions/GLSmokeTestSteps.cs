...
[Binding, Scope(Feature = "GL Smoke Test")]
public sealed class GLSmokeTestSteps
{
    private readonly ScenarioContext _scenario;
    public GLSmokeTestSteps(ScenarioContext scenario) => _scenario = scenario;
    ...
    {
        var data = _scenario.Get<ScenarioData>();
        data.GenerateRandom("LastName_0068", "^[a-z]{4}$");
        data.GenerateRandom("PrimaryPhone_0069", "[0-9]{10}");
        data.GenerateRandom("InsuredSSN", "125[0-9]{6}");
        data.GenerateRandom("AuditTelephone_0076", "[0-9]{10}");
        data.GenerateRandom("InspectionTelephone_0076", "[0-9]{10}");
        ...
        await page.PressMiddleNameAsync("Tab");
        await page.EnterLastNameAsync(data.Resolve("{{runtime:LastName_0068}}"));
        ...
        await page.PressEntityTypeAsync("Tab");
        // Source step 0069: RANDOM input for Primary Phone.
        await page.EnterPrimaryPhoneAsync(data.Resolve("{{runtime:PrimaryPhone_0069}}"));
        ...
        // Source step 0076: RANDOM input for Audit Telephone #.
        if (data.Condition("'Product (LOB)' != \"UMB\""))
        {
            await page.EnterAuditTelephoneAsync(data.Resolve("{{runtime:AuditTelephone_0076}}"));
        }
        ...
        await page.PressNameOfInspectionContactAsync("Tab");
        // Source step 0076: RANDOM input for Inspection Telephone #.
        await page.EnterInspectionTelephoneAsync(data.Resolve("{{runtime:InspectionTelephone_0076}}"));
