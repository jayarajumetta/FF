...
public sealed class WCBasicPolicyFlowPage : IFlowPage
{
    private readonly CanonicalActionExecutor _executor;
    private readonly ScenarioDataContext _data;

    public WCBasicPolicyFlowPage(CanonicalActionExecutor executor, ScenarioDataContext data)
    {
    ...
    public Task ClientAndAccountAsync(CancellationToken cancellationToken = default) =>
        _executor.ExecuteAsync(WCBasicPolicyCanonicalMap.ClientAndAccount, _data, cancellationToken);

    public Task ApplicationSetupAsync(CancellationToken cancellationToken = default) =>
        _executor.ExecuteAsync(WCBasicPolicyCanonicalMap.ApplicationSetup, _data, cancellationToken);
