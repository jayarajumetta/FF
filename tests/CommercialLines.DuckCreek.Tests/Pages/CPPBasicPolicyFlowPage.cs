...
public sealed class CPPBasicPolicyFlowPage : IFlowPage
{
    private readonly CanonicalActionExecutor _executor;
    private readonly ScenarioDataContext _data;

    public CPPBasicPolicyFlowPage(CanonicalActionExecutor executor, ScenarioDataContext data)
    {
    ...
    public Task ClientAndAccountAsync(CancellationToken cancellationToken = default) =>
        _executor.ExecuteAsync(CPPBasicPolicyCanonicalMap.ClientAndAccount, _data, cancellationToken);

    public Task ApplicationSetupAsync(CancellationToken cancellationToken = default) =>
        _executor.ExecuteAsync(CPPBasicPolicyCanonicalMap.ApplicationSetup, _data, cancellationToken);
