...
public sealed class UMBExpandedFlowPage : IFlowPage
{
    private readonly CanonicalActionExecutor _executor;
    private readonly ScenarioDataContext _data;

    public UMBExpandedFlowPage(CanonicalActionExecutor executor, ScenarioDataContext data)
    {
    ...
    public Task ClientAndAccountAsync(CancellationToken cancellationToken = default) =>
        _executor.ExecuteAsync(UMBExpandedCanonicalMap.ClientAndAccount, _data, cancellationToken);

    public Task ApplicationSetupAsync(CancellationToken cancellationToken = default) =>
        _executor.ExecuteAsync(UMBExpandedCanonicalMap.ApplicationSetup, _data, cancellationToken);
