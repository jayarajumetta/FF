...
public sealed class CPBasicPolicyFlowPage : IFlowPage
{
    private readonly CanonicalActionExecutor _executor;
    private readonly ScenarioDataContext _data;

    public CPBasicPolicyFlowPage(CanonicalActionExecutor executor, ScenarioDataContext data)
    {
    ...
    public Task ClientAndAccountAsync(CancellationToken cancellationToken = default) =>
        _executor.ExecuteAsync(CPBasicPolicyCanonicalMap.ClientAndAccount, _data, cancellationToken);

    public Task ApplicationSetupAsync(CancellationToken cancellationToken = default) =>
        _executor.ExecuteAsync(CPBasicPolicyCanonicalMap.ApplicationSetup, _data, cancellationToken);
