...
/// <summary>Stages immutable evidence in the test assembly directory before registering it with NUnit/VS Test Explorer.</summary>
public static class NUnitEvidenceAttachment
{
    ...
    public static string CreateMarker(string content)
    {
        var path = Path.Combine(GetStageDirectory(), "scenario-evidence-marker.txt");
        File.WriteAllText(path, content);
        return AddTestAttachment(path, "Scenario evidence marker");
    }
    ...
    public static string GetStageDirectory()
    {
        ...
        {
            var dir = Path.Combine(TestContext.CurrentContext.TestDirectory, "TestEvidence", key);
