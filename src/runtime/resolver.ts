...
import { enumerateFrames, type FrameRecord } from './frame-search.js';
import {
  ArrayDiagnosticSink,
...
async function visibleIndexes(locator: LocatorLike, count: number): Promise<number[]> {
  if (!locator.isVisible) return [];
  const indexes: number[] = [];
  for (let index = 0; index < count; index += 1) {
    try {
      if (await locator.nth(index).isVisible?.({ timeout: 100 })) indexes.push(index);
    } catch {
...
export async function selectStrictLocator(
  locator: LocatorLike,
  candidate: LocatorCandidate,
): Promise<{ locator: LocatorLike; index: number; count: number } | undefined> {
  const count = await locator.count();
  if (count === 0) return undefined;
  if (count === 1) return { locator, index: 0, count };
...
export async function resolveLocator(
...
): Promise<{ resolved?: ResolvedLocator; diagnostics: readonly AttemptDiagnostic[] }> {
  ...
  for (let pass = 0; pass < passes; pass += 1) {
    ...
    if (pass < passes - 1) await delay(retryDelayMs);
  }
 ...
 */
export async function performAcrossLocatorCandidates<T>(
  page: PageLike,
  spec: LocatorSpec,
  operation: (context: LocatorAttemptContext) => Promise<T>,
  options: ResolveOptions = {},
): Promise<{ value?: T; diagnostics: readonly AttemptDiagnostic[] }> {
  const localSink = options.sink ?? new ArrayDiagnosticSink();
  const candidates = buildLocatorCandidates(spec, options.candidateOptions);
  const passes = Math.max(1, options.maxPasses ?? 2);
  const retryDelayMs = Math.max(0, options.retryDelayMs ?? 75);
  ...
  for (let pass = 0; pass < passes; pass += 1) {
    for (const frame of enumerateFrames(page, spec.frame)) {
      for (const candidate of candidates) {
        let selected: Awaited<ReturnType<typeof selectStrictLocator>>;
        try {
