import {
  DEFAULT_RUNTIME_OPTIONS, LocatorCandidate, LocatorDescriptor, RuntimeOptions,
} from './model';
...
function hintScore(contextText: string, hints: string[]): { score: number; explicit: boolean } {
  let score = 0; let explicit = false;
  const normalized = contextText.toLowerCase();
  for (const hint of hints) {
    const candidate = hint.trim().toLowerCase();
    if (!candidate) continue;
    if (normalized.includes(candidate)) { score += 500; explicit = true; }
  }
  return { score, explicit };
}

function suspiciousFrame(url: string, name: string): boolean {
...
export class FrameRuntime {
  ...
  constructor(options: RuntimeOptions = {}) { this.options = { ...DEFAULT_RUNTIME_OPTIONS, ...options }; }

  async collectFrames(page: any, hints: string[] = []): Promise<FrameContext[]> {
    const contexts: FrameContext[] = [];
    const seen = new Set<any>();
    const visit = async (frame: any, depth: number): Promise<void> => {
      if (!frame || seen.has(frame) || depth > this.options.maxFrameDepth || contexts.length >= this.options.maxFrames) return;
      seen.add(frame);
      let url = ''; let name = ''; let visible = true;
      try { url = String(frame.url?.() ?? ''); } catch { /* ignored */ }
      try { name = String(frame.name?.() ?? ''); } catch { /* ignored */ }
      if (depth > 0) {
        try {
      ...
      if (!visible && !hint.explicit) score -= 300;
      if (suspiciousFrame(url, name) && !hint.explicit) score -= 450;
  ...
  private locatorFor(frame: any, candidate: LocatorCandidate): any {
    ...
    switch (candidate.kind) {
      ...
      case 'text': {
        if (candidate.tag) return frame.locator(candidate.tag).filter({ hasText: candidate.value });
  ...
  private async applyAttributeFilter(locator: any, candidate: LocatorCandidate): Promise<any[]> {
    const count = await locator.count();
    const matches: any[] = [];
    for (let index = 0; index < count; index += 1) {
      const item = locator.nth(index);
      if (!candidate.attribute || candidate.attributeValue === undefined) { matches.push(item); continue; }
      ...
      if (value === candidate.attributeValue) matches.push(item);
    }
    return matches;
  }

  private async visibleLocators(locator: any, count: number): Promise<any[]> {
    const visible: any[] = [];
    for (let index = 0; index < count; index += 1) {
      const item = locator.nth(index);
      if (await item.isVisible({ timeout: Math.min(300, this.options.candidateTimeoutMs) }).catch(() => false)) visible.push(item);
    }
    return visible;
  }

  async resolveTargets(page: any, descriptor: LocatorDescriptor): Promise<ResolvedTarget[]> {
    ...
    for (let candidateIndex = 0; candidateIndex < orderedCandidates.length; candidateIndex += 1) {
      ...
      const candidateMatches: ResolvedTarget[] = [];
      for (const frameContext of frames) {
        try {
          ...
          if (candidate.kind === 'labelAndAttribute') {
            ...
            if (filtered.length === 1) candidateMatches.push({ frameContext, locator: filtered[0], candidate, candidateIndex, count: 1 });
          ...
          if (count === 0) continue;
          if (count === 1) { candidateMatches.push({ frameContext, locator: base, candidate, candidateIndex, count }); continue; }
          ...
          if (visible.length === 1) candidateMatches.push({ frameContext, locator: visible[0], candidate, candidateIndex, count });
        } catch { /* candidate absent or invalid in this frame */ }
      }
      if (candidateMatches.length === 1) resolved.push(candidateMatches[0]);
      else if (candidateMatches.length > 1) {
        const explicit = candidateMatches.filter((match) => match.frameContext.explicitHint);
        if (explicit.length === 1) resolved.push(explicit[0]);
        else {
          const visibleFrames = candidateMatches.filter((match) => match.frameContext.visible);
          if (visibleFrames.length === 1) resolved.push(visibleFrames[0]);
  ...
  async domFallback(page: any, descriptor: LocatorDescriptor, action: DomAction): Promise<DomFallbackResult> {
    const frames = await this.collectFrames(page, descriptor.frameHints);
    const candidates = [...descriptor.candidates].sort((a, b) => b.score - a.score);
    for (const candidate of candidates) {
      for (const frameContext of frames) {
        try {
          const result = await frameContext.frame.evaluate(({ candidate: c, occurrence, action: requested }: any) => {
            const normalize = (value: unknown) => String(value ?? '').replace(/\s+/g, ' ').trim();
            const exact = (left: unknown, right: unknown) => normalize(left) === normalize(right);
            ...
            const attrCaseInsensitive = (element: Element, name: string): string | null => {
            ...
            const labelsFor = (element: Element): string[] => {
              ...
              if (id) for (const label of elements.filter((item) => item.tagName.toLowerCase() === 'label' && item.getAttribute('for') === id)) values.push(normalize(label.textContent));
              const parent = element.closest('label'); if (parent) values.push(normalize(parent.textContent));
              const aria = element.getAttribute('aria-label'); if (aria) values.push(normalize(aria));
              const labelledBy = element.getAttribute('aria-labelledby');
              if (labelledBy) for (const ref of labelledBy.split(/\s+/)) { const label = document.getElementById(ref); if (label) values.push(normalize(label.textContent)); }
              return values.filter(Boolean);
            };
            const roleOf = (element: Element): string => {
              const explicitRole = element.getAttribute('role'); if (explicitRole) return explicitRole.toLowerCase();
            ...
            const accessibleName = (element: Element): string => {
              const aria = element.getAttribute('aria-label'); if (aria) return normalize(aria);
              const labels = labelsFor(element); if (labels.length) return labels[0];
              const title = element.getAttribute('title'); if (title) return normalize(title);
              const value = (element as HTMLInputElement).value; if (value && ['button', 'submit', 'reset'].includes((element.getAttribute('type') ?? '').toLowerCase())) return normalize(value);
              return normalize(element.textContent);
            };
            const query = (): Element[] => {
              let found: Element[] = [];
              if (c.kind === 'id') found = elements.filter((element) => element.getAttribute('id') === c.value);
              else if (c.kind === 'fieldRef') found = elements.filter((element) => ['fieldref', 'data-field-ref', 'data-fieldref', 'data-dc-fieldref'].some((name) => attrCaseInsensitive(element, name) === c.value));
              else if (c.kind === 'testId') found = elements.filter((element) => attrCaseInsensitive(element, 'data-testid') === c.value);
              else if (c.kind === 'name') found = elements.filter((element) => element.getAttribute('name') === c.value);
              else if (c.kind === 'label') found = elements.filter((element) => labelsFor(element).some((label) => exact(label, c.value)));
              else if (c.kind === 'labelAndAttribute') found = elements.filter((element) => labelsFor(element).some((label) => exact(label, c.label ?? c.value)) && attrCaseInsensitive(element, c.attribute ?? '') === c.attributeValue);
              else if (c.kind === 'role') found = elements.filter((element) => roleOf(element) === String(c.role ?? '').toLowerCase() && exact(accessibleName(element), c.value));
              else if (c.kind === 'text') found = elements.filter((element) => (!c.tag || element.tagName.toLowerCase() === String(c.tag).toLowerCase()) && exact(element.textContent, c.value));
              else if (c.kind === 'css') {
                try { found = Array.from(document.querySelectorAll(c.value)); } catch { found = []; }
                ...
                try {
                  ...
                  for (let index = 0; index < snapshot.snapshotLength; index += 1) { const item = snapshot.snapshotItem(index); if (item instanceof Element) found.push(item); }
                } catch { found = []; }
              }
              return found;
            };
            const matches = query();
            if (!matches.length) return { ok: false, matched: 0, detail: 'not found' };
            let element: Element | undefined;
            if (occurrence && occurrence > 0 && occurrence <= matches.length) element = matches[occurrence - 1];
            else if (matches.length === 1) element = matches[0];
            else {
              const visible = matches.filter((item) => {
                const html = item as HTMLElement; const style = getComputedStyle(html);
                return style.display !== 'none' && style.visibility !== 'hidden' && style.opacity !== '0' && html.getClientRects().length > 0;
              });
              if (visible.length === 1) element = visible[0];
            }
            if (!element) return { ok: false, matched: matches.length, detail: 'ambiguous' };
            const html = element as HTMLElement;
            html.scrollIntoView({ block: 'center', inline: 'center' });
            const dispatch = (name: string) => element!.dispatchEvent(new Event(name, { bubbles: true, composed: true }));
            ...
            try {
              ...
              switch (requested.kind) {
                case 'click': html.click(); break;
                case 'fill': setNativeValue(element, String(requested.value ?? '')); dispatch('input'); dispatch('change'); break;
                case 'select': {
                  if (element instanceof HTMLSelectElement) {
                    ...
                    if (!option) return { ok: false, matched: matches.length, detail: 'native option not found' };
                    setNativeValue(element, option.value); dispatch('input'); dispatch('change');
                  } else {
                    html.click();
                    const wanted = normalize(requested.value).toLowerCase();
                    ...
                    if (!selected) return { ok: false, matched: matches.length, detail: 'custom option not found' };
                    (selected as HTMLElement).click();
                  }
                  break;
                }
                case 'check': if ('checked' in (element as any)) { (element as HTMLInputElement).checked = true; dispatch('input'); dispatch('change'); } else html.click(); break;
                case 'uncheck': if ('checked' in (element as any)) { (element as HTMLInputElement).checked = false; dispatch('input'); dispatch('change'); } else html.click(); break;
                case 'press': {
            ...
            } catch (error) { return { ok: false, matched: matches.length, detail: String(error) }; }
          }, { candidate, occurrence: descriptor.occurrence, action });
          if (result?.ok) return { ...result, candidate, frameContext };
